...
const Follows: React.FC = () => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      const response = await api.get(
        `user/${route.params.type}/${route.params.id}?page=${0}&limit=15`,
      );
      setShuldLoad(true);
  ...
  const handleLoadUsers = useCallback(async () => {
    ...
    try {
      const response = await api.get(
        `user/${route.params.type}/${route.params.id}?page=${page}&limit=15`,
      );
  ...
  return (
    <S.Container>
      <S.Header>
        ...
        <S.HeaderTitle>
          {route.params.type === 'getfollowers' ? 'Seguidores' : 'Seguindo'}
        </S.HeaderTitle>
      </S.Header>
      <FlatList
        data={results}
        ...
        renderItem={({ item }) => (
          <S.Item>
            <S.ItemLeft>
              <S.ItemAvatar
                source={{
                  uri: `https://lunion-mood.herokuapp.com/files/${item.avatar_url}`,
                }}
              />
            </S.ItemLeft>
            <S.ItemRight>
